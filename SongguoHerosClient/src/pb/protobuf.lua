...
--dingning, add expand mark
--	v = { __index = assert(decode_message(typename , "")) }
    v = { __index = assert(decode_message(typename , "")), __expanded = 1 }
...
-- dingning
-- expand table
function expandWholeTable(table)
    if(#(table) == 2) then
        local metaTable = getmetatable(table);
        if(metaTable and metaTable.__expanded == nil) then
            expand(table);
        end
    end
